...
static int vidxd_source_pause_device(struct vdcm_idxd *vidxd)
{
	 ...
	 * there shouldn't be any work directly submitted from the VM */
	vidxd->paused = true;
...
static int vidxd_resume_wq_state(struct vdcm_idxd *vidxd)
{
	...
	for (wq_id = 0; wq_id < vidxd->num_wqs; wq_id++) {
		...
		if (vidxd_state(vidxd) != 1 || vwqcfg->wq_state != 1) {
			...
			dev_dbg(dev, "vidxd re-enable wq %u:%u\n", wq_id, wq->id);
		...
		unsigned int *offset, bool *int_handle_revoked)
{
	...
	for (i = 1; i < VIDXD_MAX_MSIX_VECS; i++) {
		...
		*offset += sizeof(revoked_handle);
		...
		dev_dbg(dev, "%s: %d new handle %x old handle %x\n",
		...
		dev_dbg(dev, "%s: auxval %x rc %d\n", __func__, auxval, rc);
		if (rc < 0) {
			dev_dbg(dev, "set ims pasid failed rc %d\n", rc);
			break;
		}
	}
			       ...
			       loff_t *ppos)
{
	...
	default: {
		...
		dev_dbg(dev, "vendor specific region: %u\n", index);
		index -= VFIO_PCI_NUM_REGIONS;
		return vfio_pdev->region[index].ops->rw(vfio_pdev, (char __user *)buf, count, ppos, true);
	} /* end default */
	} /* end switch(index) */
