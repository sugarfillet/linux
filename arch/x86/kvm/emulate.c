...
static int em_enqcmd(struct x86_emulate_ctxt *ctxt)
{
	...
	if (dest_va & 0x3f)
		pr_warn_once("dest va is not 64 byte aligned\n");
	ctxt->op_bytes = 64;
	ctxt->src.bytes = 64;
	ctxt->dst.bytes = 64;
	...
	if (rc != X86EMUL_CONTINUE) {
		pr_info("ENQCMD: read src data failed rc %d\n", rc);
		goto done;
	}
...
static int em_enqcmds(struct x86_emulate_ctxt *ctxt)
{
	...
	ctxt->src.valptr512, ctxt->src.bytes);
	if (rc != X86EMUL_CONTINUE) {
		pr_info("ENQCMDS: read src data failed rc %d\n", rc);
		goto done;
	}
...
static int em_movdir64b(struct x86_emulate_ctxt *ctxt)
{
	...
	rc = segmented_read(ctxt, ctxt->src.addr.mem,
		ctxt->src.valptr512, ctxt->src.bytes);
	if (rc != X86EMUL_CONTINUE) {
		pr_info("MOVDIR64B: read src data failed rc %d\n", rc);
		goto done;
	}
